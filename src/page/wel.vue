<template>
  <div v-if="show">
    <basic-container>
      <div class="search">
        <el-button @click="addOrUpdate" type="primary" icon="el-icon-plus"
        >新增</el-button
        >
      </div>
      <el-table highlight-current-row :data="data">
        <template slot="empty">
          暂无数据
        </template>
        ...
        <el-table-column prop="ype" label="类型" width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.type == 1 ? "后端" : "前端" }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="starts" label="starts" width="150">
          <template slot-scope="scope">
        ...
        <el-table-column prop="address" label="码云"> </el-table-column>
        <el-table-column prop="info" label="介绍"> </el-table-column>
        <el-table-column label="操作" width="180">
          <template slot-scope="scope">
            <el-button @click="addOrUpdate(scope.row)" type="text" size="small"
            >编辑</el-button
            >
    ...
    <add-or-update
        ...
        ref="addOrUpdate"
    ></add-or-update>
    <iframe src="https://element.eleme.cn/#/zh-CN/component/form" frameborder="0" scrolling="no" style="height: 800px;width: 100%;overflow: hidden"/>
  </div>
</template>
