...
import { loginByCas, getUserInfo, getMenu, logout, getSystemOperate, getUserOperate } from '@/api/user'
...
function addPath(ele, first) {
    ...
    ele[propsDefault.children].forEach(child => {
        if (!isURL(child[propsDefault.path])) {
            let childPath=`${ele[propsDefault.path]}/${child[propsDefault.path] ? child[propsDefault.path] : "index"}`;
            if (childPath&&childPath.includes('//')){
                childPath=childPath.replace(/\/\//g, '/');
            }
            child[propsDefault.path] = childPath
        }
        addPath(child);
    })
}
